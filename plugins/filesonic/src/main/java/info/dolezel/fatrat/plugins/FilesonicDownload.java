 ...
 * @author lubos
 */
@DownloadPluginInfo(name = "FileSonic.* FREE download", regexp = "http://www\\.filesonic\\.\\w+/file/.+")
public class FilesonicDownload extends DownloadPlugin {
    ...
    static final Pattern reDownloadCode = Pattern.compile("<a href=\"([^\"]+)\"><span>Start download now!</span></a>");
    static final Pattern reHiddenField = Pattern.compile("<input type='hidden' id='[^']+' name='([^']+)' value='([^']+)' />");
    static final Pattern reFileName = Pattern.compile("\\[URL=http://www.filesonic.\\w+/file/\\d+/([^\\]]+)\\]");
        ...
        this.fetchPage(downloadLink+"?start=1", new PageFetchListener() {
            ...
            public void onCompleted(ByteBuffer buf, Map<String, String> headers) {
                ...
                if (mWait.find()) {
                    ...
                    FilesonicDownload.this.startWait(secs+1, new WaitListener() {
